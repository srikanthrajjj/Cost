...
import { ROOF_TOPIC } from "@/lib/guides/roof-cluster";

const DATE = "2026-07-27";

export function buildRoofGuideHead(input: {
  path: string;
  metaTitle: string;
    ...
    },
    {
      ...
      children: JSON.stringify(
        buildBreadcrumbList([
          ...
          { name: ROOF_TOPIC.label, path: ROOF_TOPIC.href },
          { name: input.breadcrumbTitle, path: input.path },
        ]),
      ),
    },
  ];
