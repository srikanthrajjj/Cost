...
function generateContractorQuestions(answers: KitchenEstimateAnswers): string[] {
  ...
  if (answers.remodelScope === "full") {
    ...
    questions.push(
      "How do you handle unexpected issues found during demolition (e.g., mold, outdated wiring)?",
    );
  }
  ...
  if (answers.structuralChanges.length > 0) {
    questions.push(
      "Has a structural engineer reviewed the planned wall removals or modifications?",
    );
  }
  ...
  if (answers.countertopMaterial === "marble" || answers.countertopMaterial === "granite") {
    questions.push(
      `What is the lead time for ${answers.countertopMaterial} fabrication and installation?`,
    );
  }
