...
(async () => {
  ...
  const materialInterface = typesCode.match(
    /export interface ExtractedMaterial\s*\{([^}]+)\}/s,
  )?.[1];
  ...
  const projectKnowledgeInterface = projectKnowledgeCode.match(
    /export interface ProjectKnowledge\s*\{([^}]+)\}/s,
  )?.[1];
  ...
  try {
    ...
    const extractedScopeItem = {
      name: "Drip Edge",
      description: "Metal flashing",
      quantity: 10,
      unit: "ft",
      totalPrice: 250,
    };
    ...
    for (let i = 0; i < roofingMaterials.length && !matchFound; i++) {
      ...
      console.log(
        "  extracted.name?.toLowerCase().includes(...)?",
        extractedMaterial.name?.toLowerCase().includes(knowledgeNormalized),
      );
      console.log(
        "  knowledge.name.toLowerCase().includes(...)?",
        knowledgeNormalized.includes(extractedMaterial.name?.toLowerCase()),
      );
    ...
    for (let i = 0; i < roofingScopeItems.length && !matchFound; i++) {
      ...
      console.log(
        "  extracted.name?.toLowerCase().includes(...)?",
        extractedScopeItem.name?.toLowerCase().includes(knowledgeNormalized),
      );
      console.log(
        "  knowledge.name.toLowerCase().includes(...)?",
        knowledgeNormalized.includes(extractedScopeItem.name?.toLowerCase()),
      );
